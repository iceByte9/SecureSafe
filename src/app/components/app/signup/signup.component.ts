...
isImageCaptured:boolean = false;
  ...
  onSubmit(): void {
    if (this.signupForm.valid) {
      ...
      // Store username and profile picture URL in session storage
      sessionStorage.setItem('username', formValues.username);
      sessionStorage.setItem('profilePicUrl', this.profilePicUrl as string);

      // Store captured user picture if available
      if (this.isImageCaptured) {
        const userPicture = sessionStorage.getItem('userPicture');
        console.log('userImageUrl:', userPicture);
      }
      this.router.navigate(['/homepage'], { queryParams: { username: formValues.username } });
      console.log('Form Values:', formValues);
      // You can also redirect or take any other actions after form submission
    }
  }
}
